...
def consolidar_tabela_unica(conn):
    ...
    if 'timestamp' not in df_todos.columns:
        print("  ⚠️ Coluna 'timestamp' não encontrada! Usando primeira ocorrência para time/campeonato.")
        df_mais_recente = (
            df_todos
            .drop_duplicates(subset='player_id')
            [['player_id', 'time', 'campeonato']]
            .rename(columns={'time': 'time_atual', 'campeonato': 'campeonato_atual'})
        )
    else:
        df_mais_recente = (
            df_todos
    ...
    # Colunas que não entram na agregação
    cols_identidade = {'player_id', 'nome', 'campeonato', 'time', 'nacionalidade',
                       'posicao', 'idade', 'altura'}
    ...
    df_consolidado = (
        df_todos
        .groupby(['player_id', 'nome', 'nacionalidade', 'posicao', 'idade', 'altura'])
        .agg(agg_rules)
        .reset_index()
    )

    # Substitui time e campeonato pelo mais recente
    df_consolidado = (
        df_consolidado
        .drop(columns=['campeonato', 'time'], errors='ignore')
        .merge(df_mais_recente, on='player_id', how='left')
        .rename(columns={'time_atual': 'time', 'campeonato_atual': 'campeonato'})
    )
